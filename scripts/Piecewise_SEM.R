...
p3<-ggplot(data=pointdata,aes(y=burnfreq,x=CorProtAr))+
  ...
  geom_smooth(method="glm",
              ...
              se=T)
p3
#rainfall can not go under zero so a glm model is used
...
p7<-ggplot(data=pointdata,aes(y=CorProtAr,x=elevation))+
  ...
  geom_smooth(method="glm",
              method.args=list(family=binomial),#because response variable is yes or no. 
              formula= y~x,
              se=T)
p7

# model_rainfall: rainfall predicted by elevation. #regular lm
